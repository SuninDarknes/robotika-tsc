...
float wantedBottomPos = 70,
      wantedMidPos = 40,
      wantedGripperPos = 30;


void moveToSetPosSlowly() {
  if(bottomPos== wantedBottomPos && midPos==wantedMidPos && gripperPos ==wantedGripperPos)return;

  if(bottomPos>wantedBottomPos)bottomPos--;
  else if(bottomServo<wantedBottomPos)bottomPos++;

  if(midPos>wantedMidPos)midPos--;
  else if(midPos<wantedMidPos)midPos++;

    if(gripperPos>wantedGripperPos)gripperPos--;
  else if(gripperPos<wantedGripperPos)gripperPos++;
...
*/
  ...
  newCommand += receivedData;
  if ((receivedData >= '0' && receivedData <= '9') || receivedData == '-') commandNum += receivedData;
  else if (receivedData == 'S' || receivedData == 'G' || receivedData == 'M' || receivedData == 'B' || receivedData == 'L' || receivedData == 'R') {
  ...
  */
  ...
  int recievedNumber = commandNum.toInt();
  Serial.print(newCommand);
  Serial.println(recievedNumber);
  int Ldirection, Rdirection;
  switch (newCommand[0]) {
    case 'S':
      brushlessMotor.write(recievedNumber);
      break;
    case 'G':
      gripperServo.write(recievedNumber);
      break;
    case 'M':
      midServo.write(recievedNumber);
      break;
    case 'B':
      bottomServo.write(recievedNumber);
      break;
    case 'L':
      LStepper.setMaxSpeed(abs(recievedNumber));
      LStepper.setCurrentPosition(0);
      Ldirection = recievedNumber / abs(recievedNumber);
      if (recievedNumber == 0) {
        LStepper.setCurrentPosition(0);
        LStepper.moveTo(0);
      } else if (LStepper.distanceToGo() == 0 || LStepper.distanceToGo() / (abs(LStepper.distanceToGo())) != Ldirection) {
        LStepper.setCurrentPosition(0);
        LStepper.moveTo(Ldirection * 10000);
      }
      break;
    case 'R':
      RStepper.setMaxSpeed(abs(recievedNumber));
      Rdirection = recievedNumber / abs(recievedNumber);
      if (recievedNumber == 0) {
        RStepper.setCurrentPosition(0);
        RStepper.moveTo(0);
      } else if (RStepper.distanceToGo() == 0 || RStepper.distanceToGo() / (abs(RStepper.distanceToGo())) != Rdirection) {
        RStepper.setCurrentPosition(0);
        RStepper.moveTo(Rdirection * 10000);
      }
      break;
  }
  newCommand = "Q";
